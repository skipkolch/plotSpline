...
{
    public partial class FormSpline2 : System.Windows.Forms.Form
    {
        private int N;
        private int GU_a;
        private int GU_b;
        ...
        private void Draw(double[] x, double[] y, string graphic)
        {
            ...
            {
                chart1.Series[graphic].Points.AddXY(x[i], y[i]);
            }
        }
        ...
        private double[] InitSpline_Y(double[] S_x, double[] x, double[] m, double[] F, double h)
        {
            ...
            try
            {
                for (j = 0; j < N - 1; j++)
                {
                    ...
                    localSpline_Y[j] = F[i] * F1(t) + F[i + 1] * F2(t) + m[i] * h * h * F3(t) + m[i + 1] * h * h * F4(t);
                }
